...
-> Merge Agent 在 gate 满足后合并 PR
-> Orchestrator 关闭 Issue 并写总结
```

第一版不自建完整 Web UI。GitHub Issue / PR 页面就是用户界面、任务记录、产物记录和审批记录。
...
| merge_agent | 内置确定性代码，不建议由 LLM 承载 |
...
2. 在目标仓库初始化 labels。
3. 在目标仓库添加 `.github/agent-orchestrator.yml` 策略文件。
...
repositories:
  - owner: example-org
    ...
    default_branch: main
    policy_file: .github/agent-orchestrator.yml
```

`local_path` 用于创建 worktree、运行本地 coding agent、读取测试结果。GitHub 仍然是事实来源，本地 checkout 只是执行环境。
...
- 逻辑角色：`planner`、`plan_reviewer`、`implementer`、`pr_reviewer`、`merge_agent`
...
`merge_agent` 不建议由 LLM 承载。它应该是确定性代码：检查 merge gate，调用 GitHub merge API。
  ...
  rules:
    - when:
      ...
      use:
        planner: codex
        implementer: codex
        pr_reviewer: builtin_light_review
...
- Merge Agent 不写代码、不 review，只执行 gate。
...
- PR head SHA 在 review 后变化时，Merge Agent 必须重新等待 review / CI，不得合并旧结论。
- branch protection 不满足时，Merge Agent 不绕过，标记 blocked。
