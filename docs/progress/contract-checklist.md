...
| C-SEC-001 | Permission/audit | Security Contract | All modules | Webhook signature and payload limit tests; secret redaction tests for rendered artifacts and CLI errors; GitHub App env resolution, offline JWT signing, token exchange, live-check redaction, and process env filtering tests keep secrets out of config and agent inputs | Extending for T-M6-010 |
