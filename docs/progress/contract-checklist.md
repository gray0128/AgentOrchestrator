...
| C-API-002 | Internal interface | Agent Adapter | Agent Router, role handlers | Adapter interface, fake adapter, process adapter stdin/stdout tests, routing adapter profile tests, coding CLI wrapper, env secret filtering, planner envelope, plan result, reviewer verdict tests, and multi-reviewer PR approval smoke coverage | Implemented for T-M6-005 |
...
| C-SCHEMA-004 | Data | Local Config Schema | Config loader, GitHub App auth | `npm run schema:check`; valid and invalid local-config fixture validation tests; GitHub App env-var reference tests; optional `agent_routing` catalog/profile validation tests | Implemented for T-M6-001 |
...
| C-MERGE-001 | Merge | Merge Agent | GitHub Adapter, Closeout | Merge gate, required PR approval count, Merge API, branch cleanup, final summary, issue close, and mocked end-to-end smoke tests | Implemented for T-M6-009 |
