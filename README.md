...
# 方式 1：安装到系统 PATH 目录。推荐 install：它会复制文件并设置可执行权限。
sudo install -m 755 ao /usr/local/bin/ao

# 方式 2：不使用 sudo，安装到用户目录；确保 ~/.local/bin 已在 PATH 中。
mkdir -p "$HOME/.local/bin"
install -m 755 ao "$HOME/.local/bin/ao"
...
Copy-Item .\ao.exe "$env:USERPROFILE\bin\ao.exe" -Force
& "$env:USERPROFILE\bin\ao.exe" --help
& "$env:USERPROFILE\bin\ao.exe" doctor --config config/local.json
```
...
如果使用预编译二进制，`which ao` 应指向 `/usr/local/bin/ao`、`$HOME/.local/bin/ao` 或其他 `PATH` 目录中的可执行文件。若需要升级，重新下载 release 并覆盖该文件即可。

如果使用 `npm link` 开发入口，`ao` 应指向 npm 全局 bin 下的 symlink。若 `ao` 不存在或 link 指向错误，在 AgentOrchestrator 仓库根目录重新 link：
